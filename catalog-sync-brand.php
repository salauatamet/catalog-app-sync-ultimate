<?php
/**
 * Plugin Name: Catalog.app Sync Ultimate v7.1
 * Description: Полная автоматизация с исправлением дублей атрибутов и пустых значений
 * Version: 7.1
...
class Catalog_App_Sync_V7_1 {
    ...
    private $login      = 'ВАША_ПОЧТА@gmail.com'; 
    ...
    private function update_price_cache($token) {
        $url = "{$this->api_base}/catalogs/{$this->catalog_id}/pricing-profiles/1/prices?limit=5000";
        ...
        if (is_wp_error($res)) {
            $this->log("❌ Ошибка кеша цен: " . $res->get_error_message());
            return 0;
        }
    ...
    private function process_item_v7($list_item, $token, $price_map) {
        ...
        if ($is_new && !empty($card['images']) && !$product->get_image_id()) {
    ...
    // ИСПРАВЛЕНО: Улучшенная очистка дублей с проверкой nonce
    public function ajax_clean_duplicates() {
        ...
        try {
            ...
            foreach ($duplicates as $dup) {
                // Получаем все атрибуты с этим именем
                ...
                if (count($attrs) > 1) {
                    array_shift($attrs); // Удаляем первый из массива (оставляем его в БД)
    ...
    // ИСПРАВЛЕНО: Улучшенная очистка пустых значений
    public function ajax_clean_empty() {
        ...
        try {
            ...
            // Находим пустые термины в таксономиях атрибутов
    ...
    public function render_page() {
        ...
        jQuery(document).ready(function($) {
            ...
            $('#clean-duplicates').click(function() {
                if (!confirm('Вы уверены? Это удалит дублирующиеся атрибуты.')) return;
                var btn = $(this);
                btn.prop('disabled', true).text('⏳ Очистка...');
                
                $.post(ajaxurl, {
                    action: 'clean_duplicate_attrs',
                    nonce: btn.data('nonce')
                }, function(response) {
                    if (response.success) {
                        alert('✅ ' + response.data);
                    } else {
                        alert('❌ ' + (response.data || 'Ошибка выполнения'));
                    }
            ...
            $('#clean-empty').click(function() {
                if (!confirm('Вы уверены? Это удалит пустые значения атрибутов.')) return;
                var btn = $(this);
                btn.prop('disabled', true).text('⏳ Очистка...');
                
                $.post(ajaxurl, {
                    action: 'clean_empty_attrs',
                    nonce: btn.data('nonce')
                }, function(response) {
                    if (response.success) {
                        alert('✅ ' + response.data);
        ...
        // Обработка POST-запросов с проверкой nonce
